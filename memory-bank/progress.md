...
- Temporal is integrated as a reporting-first durable state tracker for `codex/*` branch previews and dev/prod promotions. Existing GitHub Actions checks/deploy jobs and preview slot scripts remain the deployment authority; Temporal signals are best-effort except for the dedicated Temporal worker package check.
