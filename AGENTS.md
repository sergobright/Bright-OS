...
- Если во время задачи обнаружена процедурная проблема, ошибочный шаг, sandbox/escalation trap, нехватка инструмента/документации или вынужденный обходной путь, мешающий качественной работе, добавить запись в server SQLite `activities` с `activity_type_id = 'operation'`, `author = 'Codex'`, статусом `New` и заполненным `reason` с контекстом проблемы/обходным путем/что исправить; секреты не записывать. Пользовательские записи остаются `activity_type_id = 'action'`.
