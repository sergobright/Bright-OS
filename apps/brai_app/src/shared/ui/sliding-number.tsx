...
function Digit({
  ...
  place: number;
  instant: boolean;
  snapKey: number | undefined;
  snapThreshold: number;
}) {
  const valueRoundedToPlace = Math.floor(value / place) % 10;
  const initial = motionValue(valueRoundedToPlace);
  const animatedValue = useSpring(initial, TRANSITION);
  const previousSnapKeyRef = useRef(snapKey);

  useEffect(() => {
    const previousSnapKey = previousSnapKeyRef.current;
    previousSnapKeyRef.current = snapKey;

    if (instant || shouldSnapSlidingNumber(previousSnapKey, snapKey, snapThreshold)) {
      animatedValue.jump(valueRoundedToPlace);
      return;
    }

    animatedValue.set(valueRoundedToPlace);
  }, [animatedValue, instant, snapKey, snapThreshold, valueRoundedToPlace]);
...
export function shouldSnapSlidingNumber(
  previousKey: number | undefined,
  currentKey: number | undefined,
  threshold = 1,
) {
  return previousKey != null && currentKey != null && Math.abs(currentKey - previousKey) > threshold;
}
